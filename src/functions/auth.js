...
export async function saveUserInDb(idToken,user)
{
  ...
  const role=(user.role==="buyer"?"buyer":"professional");
   const result = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/create-${role}`,user,{
      headers: {
        authtoken : idToken
      }
    });
...
{
  console.log("full token",idToken);
   const result = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/read-user`,{},{
      headers: {
        authtoken : idToken
      }
    });
